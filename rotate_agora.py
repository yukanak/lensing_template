...
# CIB
cib = hp.read_map("/oak/stanford/orgs/kipac/users/yukanaka/agora_sims/agora_len_mag_cibmap_planck_545ghz_nside2048.fits")
dir_out = '/oak/stanford/orgs/kipac/users/yukanaka/agora_sims/cib/'
seed = 5001 + patch
if patch == 0:
    hp.write_map(dir_out+'agora_len_mag_cibmap_planck_545ghz_nside2048_rotated_%d.alm'%(seed),cib,overwrite=True)
else:
    r = Rotator(rot=[rotm[patch][0],rotm[patch][1],rotm[patch][2]],deg=True,inv=True)
    cib_rot = r.rotate_map_alms(cib)
    hp.write_map(dir_out+'agora_len_mag_cibmap_planck_545ghz_nside2048_rotated_%d.alm'%(seed),cib_rot,overwrite=True)
