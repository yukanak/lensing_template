...
#cib_map = hp.read_map("/oak/stanford/orgs/kipac/users/yukanaka/agora_sims/agora_len_mag_cibmap_planck_545ghz.fits")
#cib_map = hp.ud_grade(cib_map, nside)
#hp.write_map("/oak/stanford/orgs/kipac/users/yukanaka/agora_sims/agora_len_mag_cibmap_planck_545ghz_nside2048.fits", cib_map)

agora_cib = hp.read_map("/oak/stanford/orgs/kipac/users/yukanaka/agora_sims/agora_len_mag_cibmap_planck_545ghz_nside2048.fits")
clii = hp.anafast(agora_cib)
agora_input_klm = hp.read_map("/oak/stanford/orgs/kipac/users/yukanaka/agora_sims/agora_spt3g_input_kappa_map_nside2048.fits")
clik = hp.anafast(agora_cib,agora_input_klm)
np.savetxt(f"/oak/stanford/orgs/kipac/users/yukanaka/lensing_template/agora545/cls/cls_clii_545ghz_fullsky.dat", clii)
np.savetxt(f"/oak/stanford/orgs/kipac/users/yukanaka/lensing_template/agora545/cls/cls_clik_545ghz_inputkappa_fullsky.dat", clik)
...
plt.plot(np.arange(2,lmax+1), clik[:lmax+1], color='lightcoral', alpha=0.5, linestyle='--', label='Agora 545 GHz clik with input kappa')
#plt.plot(np.arange(2,lmax+1), clkk[:lmax+1], color='cornflowerblue', alpha=0.5, linestyle='--', label='clkk')
plt.plot(np.arange(2,lmax+1), clii[:lmax+1], color='orange', alpha=0.5, linestyle='--', label='Agora 545 GHz clii')
plt.yscale('log')
...
plt.xlim(10,2000)
plt.tight_layout()
plt.savefig('/home/users/yukanaka/lensing_template/figs/agora_545ghz_cib_input_kappa_specs.png')
